...
const METRIC_LABELS: Record<string, string> = {
  ...
  accepted_scientific_name: 'Accepted Name',
  region: 'Region',
  county: 'County',
  city: 'City',
  continent: 'Continent',
  ...
  observed_on_string: 'Observed On',
  positional_accuracy_m: 'Position Accuracy (m)',
  photo_url: 'Photo',
  identifications_count: 'Identifications',
  num_identification_agreements: 'ID Agreements',
...
const FAMILY_METRIC_BUILDERS: Record<DataFamily, MetricBuilder> = {
  ...
  biodiversity: (metrics) => pick([
    ...
    metrics.iconic_taxon ? { label: 'Iconic Taxon', value: String(metrics.iconic_taxon) } : null,
    metrics.quality_grade ? { label: 'Quality', value: String(metrics.quality_grade) } : null,
    num(metrics, 'observation_count') !== null ? { label: 'Observations', value: num(metrics, 'observation_count')!.toFixed(0) } : null,
    metrics.locality ? { label: 'Locality', value: String(metrics.locality) } : null,
    metrics.event_date ? { label: 'Event Date', value: String(metrics.event_date) } : null,
    metrics.recorded_by ? { label: 'Recorded By', value: String(metrics.recorded_by) } : null,
    metrics.region ? { label: 'Region', value: String(metrics.region) } : null,
    metrics.kingdom ? { label: 'Kingdom', value: String(metrics.kingdom) } : null,
    metrics.threatened === true ? { label: 'Threatened', value: 'Yes' } : null,
    num(metrics, 'depth') !== null ? { label: 'Depth', value: `${num(metrics, 'depth')!.toFixed(0)} m` } : null,
  ]),
