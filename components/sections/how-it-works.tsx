...
// --- Background shader (morphing multicolour ring) --- //
// Previously blank on production because backgroundColor lived in the WebGL
// effect deps as a new array every MutationObserver tick, so the GL context
// was torn down/rebuilt in a loop and never painted a stable frame. Init once;
// only the uniform is updated when the theme changes.
...
const ShaderCanvas = () => {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.error('WebGL not supported — HowItWorks ring animation disabled')
      return
    }
    glRef.current = gl
    ...
    const fragmentShaderSource = `
      ...
      void main(){
        ...
        float xScale = mix(1.0, 2.1, widen);
    ...
    const applyBackground = () => {
      // This section is always dark-styled on the homepage; keep the ring on black
      // so theme class flicker cannot wipe the animation.
      const loc = glBgColorLocationRef.current
      if (!loc) return
      gl.useProgram(program)
      gl.uniform3fv(loc, new Float32Array([0, 0, 0]))
    }
    applyBackground()
    ...
    handleResize()
    // First layout can report 0×0; observe the canvas box until it has size.
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden
      className="pointer-events-none absolute inset-0 z-0 h-full w-full block"
    />
  )
}
...
const WorkflowStepCard = ({
  ...
  <div
    className="
      backdrop-blur-[14px] bg-gradient-to-br rounded-2xl shadow-xl w-full max-w-full md:max-w-xs
      px-7 py-8 flex flex-col transition-all duration-300
      from-black/5 to-black/0 border border-black/10
      dark:from-white/10 dark:to-white/5 dark:border-white/10 dark:backdrop-brightness-[0.91]
    "
  >
    <div className="flex items-center gap-3 mb-4">
      <div className="w-12 h-12 shrink-0 rounded-xl bg-gradient-to-br from-cyan-400/25 to-blue-500/20 border border-cyan-400/25 flex items-center justify-center shadow-[0_0_24px_-8px_rgba(34,211,238,0.55)]">
        {icon}
      </div>
    ...
    <ul className="flex flex-col gap-2 text-[14px] text-foreground/90 font-sans">
...
export function HowItWorks() {
  const workflowSteps: WorkflowStepProps[] = [
    {
      ...
      icon: <Satellite className="h-6 w-6 text-cyan-300" strokeWidth={1.75} />,
      features: [
    ...
    },
    {
      ...
      icon: <Cpu className="h-6 w-6 text-cyan-300" strokeWidth={1.75} />,
      features: [
    ...
    },
    {
      ...
      icon: <Link2 className="h-6 w-6 text-cyan-300" strokeWidth={1.75} />,
      features: [
    ...
    },
    {
      ...
      icon: <LayoutDashboard className="h-6 w-6 text-cyan-300" strokeWidth={1.75} />,
      features: [
  ...
  return (
    <section
      id="how-it-works"
      className="relative min-h-screen w-full overflow-x-hidden py-20 scroll-mt-24"
    >
      <ShaderCanvas />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-20">
          <div
            className="backdrop-blur-[14px] bg-gradient-to-br rounded-2xl shadow-xl max-w-4xl mx-auto px-8 py-10
            from-black/5 to-black/0 border border-black/10
            dark:from-white/10 dark:to-white/5 dark:border-white/10 dark:backdrop-brightness-[0.91]"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-cyan-400/25 to-blue-500/20 border border-cyan-400/25 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <Shield className="h-8 w-8 text-cyan-300" strokeWidth={1.75} />
                </div>
              ...
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-cyan-400/25 to-blue-500/20 border border-cyan-400/25 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <Eye className="h-8 w-8 text-cyan-300" strokeWidth={1.75} />
                </div>
              ...
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-cyan-400/25 to-blue-500/20 border border-cyan-400/25 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <BadgeCheck className="h-8 w-8 text-cyan-300" strokeWidth={1.75} />
                </div>
                <h4 className="font-medium text-cyan-400 mb-2">Trust</h4>
                <p className="text-sm text-foreground/70">
                  No single point of failure or data manipulation
                </p>
              </div>
            </div>
          </div>
