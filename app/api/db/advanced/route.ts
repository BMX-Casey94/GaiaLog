...
export async function GET(req: NextRequest) {
  try {
    ...
    const sql = `SELECT id, provider, city, lat, lon, uv_index, soil_moisture_pct, wildfire_risk, environmental_score,
    ...
    const [rows, total] = await Promise.all([
      ...
      fastTableCount('advanced_metrics_readings', whereSql, params),
    ])
