...
export async function GET(req: NextRequest) {
  try {
    ...
    if (idParam && /^\d+$/.test(idParam)) {
      ...
      whereSql = 'WHERE station_code ILIKE $1 OR provider ILIKE $1 OR txid ILIKE $1 OR location ILIKE $1'
      params.push(`%${q}%`)
    }
    const orderBy = (() => {
      switch (sort) {
    ...
    const sql = `SELECT id, provider, station_code, location, lat, lon, level_m, tide_height_m, wave_height_m,
    ...
    const [rows, total] = await Promise.all([
      ...
      fastTableCount('water_level_readings', whereSql, params),
    ])
