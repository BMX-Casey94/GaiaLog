...
import { bsvStatsCache } from '@/lib/stats-cache'

export async function GET() {
  try {
    // Use caching for BSV stats (15s TTL)
    const cachedStats = await bsvStatsCache.getStats(async () => {
    ...
    const cacheAge = bsvStatsCache.getCacheAge()
    ...
    const stale = bsvStatsCache.getStale()
    if (stale) {
      const httpMetrics = fetchMetricsStore.snapshot()
      return NextResponse.json({
        success: true,
        ...
        cached: true,
        stale: true,
        cacheAge: bsvStatsCache.getCacheAge()
      })
    }
